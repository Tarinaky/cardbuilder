...
class Template(object):
    def __init__(self, resources_path, template_path):
        ...
        self.border = pygame.image.load(
            template_path.parents[0].joinpath(template_json["border"])
        )
    ...
    def render_card_name(self, surface, name):
        ...
        if name_surface.get_width() > width:
            name_surface = pygame.transform.smoothscale(
                name_surface,
                (width, width / name_surface.get_width() * name_surface.get_height()),
            )
    ...
    def render_cost(self, surface, cost):
        ...
        if data[0].lower() == "blood":
            ...
            cost_surface = pygame.font.Font(self.font, 48).render(
                data[1], True, (120, 0, 0)
            )
    ...
    def render_sigils(self, surface, sigil_list, sigils):
        ...
        for (sigil, i) in zip(sigils, itertools.count(0)):
            ...
            sigil_surface = font.render(
                sigil_list.get(sigil).initials, True, self.text_color
            )
            ...
            if len(sigils) > 1:
                sigil_surface = pygame.transform.smoothscale(
                    sigil_surface,
                    (
                        sigil_surface.get_width() * 3 / 5,
                        sigil_surface.get_height() * 3 / 5,
                    ),
                )
            surface.blit(
                sigil_surface,
                (x - sigil_surface.get_width() / 2, y - sigil_surface.get_height() / 2),
            )
    ...
    def render_text(self, surface, trigger, sigil_list, sigils):
        if trigger is not None:
            ...
            self._render_multiline(
                surface, font, self.helptext_loc, self.text_color, trigger
            )
            ...
            self._render_multiline(
                surface, font, self.helptext_loc, self.text_color, text
            )
        elif len(sigils) == 1:
            ...
            self._render_multiline(
                ...
                self.helptext_loc,
                self.helptext_color,
                sigil_list.get(sigils[0]).helptext,
            )
        else:
            ...
            if len(forced) > 0:
                ...
                self._render_multiline(
                    ...
                    self.helptext_loc,
                    self.helptext_color,
                    sigil_list.get(forced[0]).helptext,
                )
